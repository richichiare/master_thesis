...
\usepackage{hyperref}
\hypersetup{
  pdftitle = {DTA-Tracker: A colourful approach to understanding split-personality malware},%
  pdfauthor = {Riccardo Chiaretti},%
...
\noindent
From that set (see \iffalse ref al listing in cui c'è il log bluepill\fi ), it is possible to select those that will be the taint sources in later executions of {\sf DTA-Tracker}. For the evaluations provided in Section~\ref{sec:evaluations}, we ran BluePill on Furtim malware and we chose the following set of tainted sources:
\begin{itemize}
